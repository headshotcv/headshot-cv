...
router.get("/api/jobs/:id/cv.pdf", async (req, res) => {
  const parsed = GetJobParams.safeParse(req.params);
  if (!parsed.success) {
    return res.status(400).json({ error: "Invalid params" });
  }
...
router.get("/api/jobs/:id/photo.png", async (req, res) => {
  const parsed = GetJobParams.safeParse(req.params);
  if (!parsed.success) {
    return res.status(400).json({ error: "Invalid params" });
  }
...
router.post("/api/jobs/:id/confirm-payment", async (req, res) => {
  ...
  await db
    ...
    .set({
      status: "processing",
      stripePaymentIntentId: bodyParsed.data.stripePaymentIntentId,
      updatedAt: new Date(),
    })
